...
class ChatBot extends Component {
  ...
  componentDidMount() {
    ...
    if (this.content) {
      this.content.current.addEventListener('DOMNodeInserted', this.onNodeInserted);
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    const { steps: prevSteps } = this.props;
    const { previousSteps, previousStep, renderedSteps } = nextState;

    if (opened !== undefined && opened !== nextState.opened) {
      this.setState({ opened });
    }

    if (reset !== undefined && reset !== nextState.reset) {
      this.setState({ reset });
      // this.getSteps(this.props);
    }

    if (steps !== prevSteps) {
      let newStep = [...steps].pop();
      let settings = {};
      ...
      } else if (newStep.message || newStep.asMessage) {
        settings = defaultBotSettings;
      } else if (newStep.component) {
        settings = defaultCustomSettings;
      }

      newStep = {
        [newStep.id]: Object.assign({}, settings, schema.parse(newStep)),
      };

      schema.checkInvalidIds(newStep);

      const updatedSteps = { ...nextState.steps, ...newStep };
      const currentStep = [...steps].pop();
      const lastRendered = [...renderedSteps].pop();
      let curRendered = [...renderedSteps];

      if (lastRendered && !lastRendered.id) {
        curRendered = curRendered.slice(0, -1);
      }
      ...
      this.setState({
        ...
        previousStep,
        previousSteps,
        renderedSteps: [...curRendered, currentStep],
        steps: updatedSteps,
      });
    }
  }
  ...
  componentWillUnmount() {
    if (this.content) {
      this.content.current.removeEventListener('DOMNodeInserted', this.onNodeInserted);
